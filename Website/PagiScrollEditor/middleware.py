from django.template.response import TemplateResponse
from django.utils.translation import gettext as _
from django.db.models import Q
from .utils import GetCustomFilterPage
from Main.utils import getAllWithTags
from Post.models import Tag, Article, Tool, Note, Service

class PagiScrollEditorMiddleware:
    def __init__(self, get_response):
        self.get_response = get_response
        self.allowed_templates = ('PagiScroll/base_post_list.html',)

    def __call__(self, request):
        response = self.get_response(request)
        return response
    
    def process_view(self, request, view_func, view_args, view_kwargs):
        return None

    def process_exception(self, request, exception):
        pass

    def process_template_response(self, request, response: TemplateResponse):
        if response.template_name in self.allowed_templates:
            # Here you will be check if URL match with slug of custom filter page
            filter_page = GetCustomFilterPage(request.get_full_path())
            category_slug = response.context_data.get('category').slug
            category = None
            match(category_slug):
                case 'articles':
                    category = Article
                case 'tools':
                    category = Tool
                case 'notes':
                    category = Note
                case 'services':
                    category = Service

            tags = response.context_data.get('tags_json')
            if filter_page is not None:
                tag_obj = Tag.objects.filter(Q(name_ru__in=tags) | Q(name_en__in=tags) | Q(slug_ru__in=tags)  | Q(slug_en__in=tags))
                response.context_data.update({'custom_fp': True})
                #response.context_data.update({'posts': getAllWithTags(category.objects.all(), tag_obj)})
                response.context_data.update({'cfp_title': filter_page.title})
                response.context_data.update({'cfp_description': filter_page.description})
                response.context_data.update({'cfp_h1': filter_page.h1})
                response.context_data.update({'cfp_lead': filter_page.lead})
                response.context_data.update({'cfp_conclusion': filter_page.conclusion})

        return response