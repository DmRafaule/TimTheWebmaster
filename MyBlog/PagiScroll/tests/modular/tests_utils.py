...
class UtilsCurrentTimingTest(TestCase):
    ''' Специальный класс для тестирования записей в БД, с различными таймингом создания '''
    def setUp(self):
        # Создаём категории
        self.article_cat = Category(slug='articles', name_ru="Статьи", name_en="Articles", description_ru="Описание статей", description_en="Articles\' description" )
        ...
        self.note_cat.save()
        # Генерируем посты различных типов
        articles = self.generate_article_queryset(0,5)
        tools = self.generate_tool_queryset(0,5)
        notes = self.generate_note_queryset(0,5)
        self.art1 = articles[0]
        self.art1.timeCreated = self.art1.timeCreated.replace(year=2020, month=1, day=1)#wen
        self.art1.save()
        self.art2 = articles[1]
    ...
    def test_get_this_year_articles(self):
        ''' Получение статей в этом году '''
        queryset = P_utils.get_this_year_posts(Article.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_month_articles(self):
        ''' Получение статей в этом месяце '''
        queryset = P_utils.get_this_month_posts(Article.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_week_articles(self):
        ''' Получение статей на этой неделе '''
        queryset = P_utils.get_this_week_posts(Article.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_day_articles(self):
        ''' Получение статей за этот день '''
        queryset = P_utils.get_this_day_posts(Article.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_year_tools(self):
        ''' Получение инструментов в этом году '''
        queryset = P_utils.get_this_year_posts(Tool.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_month_tools(self):
        ''' Получение инструментов в этом месяце '''
        queryset = P_utils.get_this_month_posts(Tool.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_week_tools(self):
        ''' Получение инструментов на этой неделе '''
        queryset = P_utils.get_this_week_posts(Tool.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_day_tools(self):
        ''' Получение инструментов за этот день '''
        queryset = P_utils.get_this_day_posts(Tool.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_year_notes(self):
        ''' Получение заметок в этом году '''
        queryset = P_utils.get_this_year_posts(Note.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_month_notes(self):
        ''' Получение заметок в этом месяце '''
        queryset = P_utils.get_this_month_posts(Note.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_week_notes(self):
        ''' Получение заметок на этой неделе '''
        queryset = P_utils.get_this_week_posts(Note.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")

    def test_get_this_day_notes(self):
        ''' Получение заметок за этот день '''
        queryset = P_utils.get_this_day_posts(Note.objects.all())
        self.assertEqual(len(queryset), 2, msg=f"In queryset: {queryset}")